...
$alert = $_GET['update'] ?? null;

// get data
$db = (new SQLiteConnection())->connect("../../db/database.db");
$setting = $db->query("SELECT * FROM settings WHERE id = 1")->fetchAll()[0] ?? null;
...
<body class="flex flex-row">
    ...
    <div class="py-5 px-10 w-max">
        ...
        <div class="flex items-center justify-between">
            <h3 class="font-semibold text-2xl">Pengaturan</h3>
        </div>
        <div class="container bg-gray-800 mt-5 p-5 rounded-md w-[500px]">
            <?php
            if ($alert == 'success') {
                echo "<h3 class='text-emerald-500 text-sm mb-5 ml-1'>Data berhasil diperbarui.</h3>";
            }
            ?>

            <form action="update.php" method="post">
                <label class="form-control w-full">
                    <div class="label"><span class="label-text">Nomor WhatsApp Penjualan</span></div>
                    <input name="wa" type="text" placeholder="08..." value="<?= $setting['no_wa'] ?>" class="input input-bordered" />
                </label>
